# Evidence Index (Auto)
Generated: 2025-12-30T01:53:24.678Z

| Merged | PR | MILESTONE_ID | BLUEPRINT_REF | Evidence |
|---|---|---|---|---|
| 2025-12-30 | [#70](https://github.com/natbkgift/flowbiz-client-ai-builder/pull/70) | PR-XXX | BLUEPRINT — External Memory update | 1 |
| 2025-12-30 | [#69](https://github.com/natbkgift/flowbiz-client-ai-builder/pull/69) | PR-XXX | BLUEPRINT — External Memory update | 1 |
| 2025-12-30 | [#68](https://github.com/natbkgift/flowbiz-client-ai-builder/pull/68) | PR-XXX | BLUEPRINT — External Memory update | 1 |
| 2025-12-30 | [#67](https://github.com/natbkgift/flowbiz-client-ai-builder/pull/67) | HOTFIX-SECURITY-SCAN | BLUEPRINT — SAFE / CONTROLLED HALT | 1 |
| 2025-12-30 | [#66](https://github.com/natbkgift/flowbiz-client-ai-builder/pull/66) | PR-017 | BLUEPRINT — PR-17: Webhook Watcher & Notifications | 2 |
| 2025-12-29 | [#65](https://github.com/natbkgift/flowbiz-client-ai-builder/pull/65) | PR-XXX | BLUEPRINT — External Memory baseline | 0 |
| 2025-12-29 | [#64](https://github.com/natbkgift/flowbiz-client-ai-builder/pull/64) | PR-016 | BLUEPRINT — Milestone Index / PR-016 — Policy Enforcer (Revert) | 1 |
| 2025-12-29 | [#62](https://github.com/natbkgift/flowbiz-client-ai-builder/pull/62) | PR-15 | BLUEPRINT — PR-15: Evidence Model + Artifact Registry | 1 |
| 2025-12-29 | [#59](https://github.com/natbkgift/flowbiz-client-ai-builder/pull/59) | PR-059 | BLUEPRINT — Governance Controls / Reference Style Remediation | 3 |
| 2025-12-29 | [#55](https://github.com/natbkgift/flowbiz-client-ai-builder/pull/55) | _pending_ | _pending_ | 1 |
| 2025-12-28 | [#48](https://github.com/natbkgift/flowbiz-client-ai-builder/pull/48) | PR-35 | Section 11 / PR-35 | 1 |
| 2025-12-28 | [#49](https://github.com/natbkgift/flowbiz-client-ai-builder/pull/49) | PR-###            # Required for MILESTONE | Section 11 / PR-### | 1 |
| 2025-12-28 | [#47](https://github.com/natbkgift/flowbiz-client-ai-builder/pull/47) | PR-35 | Section 11 / PR-35 | 0 |
| 2025-12-28 | [#46](https://github.com/natbkgift/flowbiz-client-ai-builder/pull/46) | PR-35 | Section 11 / PR-35 | 0 |
| 2025-12-28 | [#42](https://github.com/natbkgift/flowbiz-client-ai-builder/pull/42) | _missing_ | _missing_ | 0 |
| 2025-12-28 | [#41](https://github.com/natbkgift/flowbiz-client-ai-builder/pull/41) | _missing_ | _missing_ | 2 |
| 2025-12-28 | [#39](https://github.com/natbkgift/flowbiz-client-ai-builder/pull/39) | _missing_ | _missing_ | 2 |
| 2025-12-28 | [#30](https://github.com/natbkgift/flowbiz-client-ai-builder/pull/30) | _missing_ | _missing_ | 0 |
| 2025-12-28 | [#25](https://github.com/natbkgift/flowbiz-client-ai-builder/pull/25) | _missing_ | _missing_ | 0 |
| 2025-12-28 | [#24](https://github.com/natbkgift/flowbiz-client-ai-builder/pull/24) | _missing_ | _missing_ | 0 |
| 2025-12-28 | [#15](https://github.com/natbkgift/flowbiz-client-ai-builder/pull/15) | PR-14 | PR #14 — Gate Framework v1 | 2 |
| 2025-12-28 | [#14](https://github.com/natbkgift/flowbiz-client-ai-builder/pull/14) | _missing_ | _missing_ | 1 |
| 2025-12-27 | [#13](https://github.com/natbkgift/flowbiz-client-ai-builder/pull/13) | _missing_ | _missing_ | 4 |
| 2025-12-27 | [#12](https://github.com/natbkgift/flowbiz-client-ai-builder/pull/12) | _missing_ | _missing_ | 1 |
| 2025-12-27 | [#8](https://github.com/natbkgift/flowbiz-client-ai-builder/pull/8) | _missing_ | _missing_ | 1 |
| 2025-12-27 | [#6](https://github.com/natbkgift/flowbiz-client-ai-builder/pull/6) | _missing_ | _missing_ | 2 |
| 2025-12-27 | [#5](https://github.com/natbkgift/flowbiz-client-ai-builder/pull/5) | _missing_ | _missing_ | 1 |
| 2025-12-27 | [#4](https://github.com/natbkgift/flowbiz-client-ai-builder/pull/4) | _missing_ | _missing_ | 0 |


## PR #70: Governance — AUTO_RUN Standing Defaults
- MILESTONE_ID: PR-XXX
- BLUEPRINT_REF: BLUEPRINT — External Memory update
- PR: https://github.com/natbkgift/flowbiz-client-ai-builder/pull/70

**Evidence Links**
- https://github.com/natbkgift/flowbiz-client-ai-builder/pull/70

---

## PR #69: External Memory sync (PROJECT_STATE/DEFERRED/AUTORUN)
- MILESTONE_ID: PR-XXX
- BLUEPRINT_REF: BLUEPRINT — External Memory update
- PR: https://github.com/natbkgift/flowbiz-client-ai-builder/pull/69

**Evidence Links**
- https://github.com/natbkgift/flowbiz-client-ai-builder/pull/69

---

## PR #68: External Memory update (PROJECT_STATE/DEFERRED/AUTORUN)
- MILESTONE_ID: PR-XXX
- BLUEPRINT_REF: BLUEPRINT — External Memory update
- PR: https://github.com/natbkgift/flowbiz-client-ai-builder/pull/68

**Evidence Links**
- https://github.com/natbkgift/flowbiz-client-ai-builder/pull/68

---

## PR #67: HOTFIX — Security Scan
- MILESTONE_ID: HOTFIX-SECURITY-SCAN
- BLUEPRINT_REF: BLUEPRINT — SAFE / CONTROLLED HALT
- PR: https://github.com/natbkgift/flowbiz-client-ai-builder/pull/67

**Evidence Links**
- https://github.com/natbkgift/flowbiz-client-ai-builder/actions

---

## PR #66: PR-17: Webhook Watcher & Notifications
- MILESTONE_ID: PR-017
- BLUEPRINT_REF: BLUEPRINT — PR-17: Webhook Watcher & Notifications
- PR: https://github.com/natbkgift/flowbiz-client-ai-builder/pull/66

**Evidence Links**
- https://github.com/natbkgift/flowbiz-client-ai-builder/pull/66
- https://github.com/natbkgift/flowbiz-client-ai-builder/actions/runs/20585098235

---

## PR #64: BLUEPRINT — Milestone Index / PR-016 — Policy Enforcer (Revert)
- MILESTONE_ID: PR-016
- BLUEPRINT_REF: BLUEPRINT — Milestone Index / PR-016 — Policy Enforcer (Revert)
- PR: https://github.com/natbkgift/flowbiz-client-ai-builder/pull/64

**Evidence Links**
- https://github.com/natbkgift/flowbiz-client-ai-builder/pull/64

---

## PR #65: PR-XXX — External Memory baseline (PROJECT_STATE/DEFERRED/AUTORUN)
- MILESTONE_ID: PR-XXX
- BLUEPRINT_REF: BLUEPRINT — External Memory baseline
- PR: https://github.com/natbkgift/flowbiz-client-ai-builder/pull/65

**Evidence Links**
- https://github.com/natbkgift/flowbiz-client-ai-builder/blob/feature/pr-external-memory-baseline/PROJECT_STATE.md
- https://github.com/natbkgift/flowbiz-client-ai-builder/blob/feature/pr-external-memory-baseline/DEFERRED_PRs.md
- https://github.com/natbkgift/flowbiz-client-ai-builder/blob/feature/pr-external-memory-baseline/AUTORUN_DECISIONS.md

---

## PR #48: PR-35: Evidence Index update (auto)
- MILESTONE_ID: PR-35
- BLUEPRINT_REF: Section 11 / PR-35
- PR: https://github.com/natbkgift/flowbiz-client-ai-builder/pull/48

**Evidence Links**
- https://github.com/natbkgift/flowbiz-client-ai-builder/pull/48

## PR #49: Add files via upload
- MILESTONE_ID: PR-###            # Required for MILESTONE
- BLUEPRINT_REF: Section 11 / PR-###
- PR: https://github.com/natbkgift/flowbiz-client-ai-builder/pull/49

**Evidence Links**
- https://github.com/natbkgift/flowbiz-client-ai-builder/pull/49

## PR #47: PR-35: Evidence Index update (auto)
- MILESTONE_ID: PR-35
- BLUEPRINT_REF: Section 11 / PR-35
- PR: https://github.com/natbkgift/flowbiz-client-ai-builder/pull/47

**Evidence Links**
_none_

## PR #46: PR-35: Evidence Index update (auto)
- MILESTONE_ID: PR-35
- BLUEPRINT_REF: Section 11 / PR-35
- PR: https://github.com/natbkgift/flowbiz-client-ai-builder/pull/46

**Evidence Links**
_none_

## PR #42: FlowBiz_Audit_Pack_FULL
- MILESTONE_ID: _missing_
- BLUEPRINT_REF: _missing_
- PR: https://github.com/natbkgift/flowbiz-client-ai-builder/pull/42

**Evidence Links**
_none_

## PR #41: [WIP] Build client service to integrate with core system
- MILESTONE_ID: _missing_
- BLUEPRINT_REF: _missing_
- PR: https://github.com/natbkgift/flowbiz-client-ai-builder/pull/41

**Evidence Links**
- https://github.com/natbkgift/flowbiz-template-service
- https://github.com/natbkgift/flowbiz-client-ai-builder/issues/new?title=✨+Set+up+Copilot+instructions&body=Configure%20instructions%20for%20this%20repository%20as%20documented%20in%20%5BBest%20practices%20for%20Copilot%20coding%20agent%20in%20your%20repository%5D%28https://gh.io/copilot-coding-agent-tips%29%2E%0A%0A%3COnboard%20this%20repo%3E&assignees=copilot

## PR #39: Update PR description to align with Blueprint metadata requirements
- MILESTONE_ID: _missing_
- BLUEPRINT_REF: _missing_
- PR: https://github.com/natbkgift/flowbiz-client-ai-builder/pull/39

**Evidence Links**
- https://github.com/natbkgift/flowbiz-template-service
- https://github.com/natbkgift/flowbiz-client-ai-builder/issues/new?title=✨+Set+up+Copilot+instructions&body=Configure%20instructions%20for%20this%20repository%20as%20documented%20in%20%5BBest%20practices%20for%20Copilot%20coding%20agent%20in%20your%20repository%5D%28https://gh.io/copilot-coding-agent-tips%29%2E%0A%0A%3COnboard%20this%20repo%3E&assignees=copilot

## PR #30: Restore required workflows and align check names
- MILESTONE_ID: _missing_
- BLUEPRINT_REF: _missing_
- PR: https://github.com/natbkgift/flowbiz-client-ai-builder/pull/30

**Evidence Links**
_none_

## PR #25: Revert "Refactor AUTO_RUN workflow for main branch CI"
- MILESTONE_ID: _missing_
- BLUEPRINT_REF: _missing_
- PR: https://github.com/natbkgift/flowbiz-client-ai-builder/pull/25

**Evidence Links**
_none_

## PR #24: Refactor AUTO_RUN workflow for main branch CI
- MILESTONE_ID: _missing_
- BLUEPRINT_REF: _missing_
- PR: https://github.com/natbkgift/flowbiz-client-ai-builder/pull/24

**Evidence Links**
_none_

## PR #15: PR-14: Gate Framework v1
- MILESTONE_ID: PR-14
- BLUEPRINT_REF: PR #14 — Gate Framework v1
- PR: https://github.com/natbkgift/flowbiz-client-ai-builder/pull/15

**Evidence Links**
- https://github.com/natbkgift/flowbiz-template-service
- https://gh.io/copilot-coding-agent-survey

## PR #14: Implement PR 11-12 requirements: CI workflow, PR_POLICY.md, and RUNBOOK.md
- MILESTONE_ID: _missing_
- BLUEPRINT_REF: _missing_
- PR: https://github.com/natbkgift/flowbiz-client-ai-builder/pull/14

**Evidence Links**
- https://gh.io/copilot-coding-agent-survey

## PR #13: GitHub Adapter v1 — PR Creation & Status Reading
- MILESTONE_ID: _missing_
- BLUEPRINT_REF: _missing_
- PR: https://github.com/natbkgift/flowbiz-client-ai-builder/pull/13

**Evidence Links**
- https://github.enterprise.com/api/v3
- https://github.com"
- https://api.github.com`
- https://github.com/natbkgift/flowbiz-client-ai-builder/issues/new?title=✨+Set+up+Copilot+instructions&body=Configure%20instructions%20for%20this%20repository%20as%20documented%20in%20%5BBest%20practices%20for%20Copilot%20coding%20agent%20in%20your%20repository%5D%28https://gh.io/copilot-coding-agent-tips%29%2E%0A%0A%3COnboard%20this%20repo%3E&assignees=copilot

## PR #12: Add auto-approval for workflow approvals
- MILESTONE_ID: _missing_
- BLUEPRINT_REF: _missing_
- PR: https://github.com/natbkgift/flowbiz-client-ai-builder/pull/12

**Evidence Links**
- https://gh.io/copilot-coding-agent-survey

## PR #8: AUTO_RUN CONTROL — Automated milestone progression with FAIL_SAFE recovery
- MILESTONE_ID: _missing_
- BLUEPRINT_REF: _missing_
- PR: https://github.com/natbkgift/flowbiz-client-ai-builder/pull/8

**Evidence Links**
- https://github.com/natbkgift/flowbiz-client-ai-builder/issues/new?title=✨+Set+up+Copilot+instructions&body=Configure%20instructions%20for%20this%20repository%20as%20documented%20in%20%5BBest%20practices%20for%20Copilot%20coding%20agent%20in%20your%20repository%5D%28https://gh.io/copilot-coding-agent-tips%29%2E%0A%0A%3COnboard%20this%20repo%3E&assignees=copilot

## PR #6: PR #3: Core contracts for AI Builder platform (types only)
- MILESTONE_ID: _missing_
- BLUEPRINT_REF: _missing_
- PR: https://github.com/natbkgift/flowbiz-client-ai-builder/pull/6

**Evidence Links**
- https://github.com/natbkgift/flowbiz-template-service
- https://gh.io/copilot-coding-agent-tips

## PR #5: PR #3: Core contracts for AI Builder platform (types only)
- MILESTONE_ID: _missing_
- BLUEPRINT_REF: _missing_
- PR: https://github.com/natbkgift/flowbiz-client-ai-builder/pull/5

**Evidence Links**
- https://gh.io/copilot-coding-agent-tips

## PR #4: Merge pull request #2 from natbkgift/copilot/update-pull-request-temp…
- MILESTONE_ID: _missing_
- BLUEPRINT_REF: _missing_
- PR: https://github.com/natbkgift/flowbiz-client-ai-builder/pull/4

**Evidence Links**
_none_

## PR #52: Update governance documents for project compliance
- MILESTONE_ID: _pending_
- BLUEPRINT_REF: _pending_
- PR: https://github.com/natbkgift/flowbiz-client-ai-builder/pull/52

**Evidence Links**
- https://github.com/natbkgift/flowbiz-client-ai-builder/pull/52

## PR #55: Add agent-next-pr-ready compliance gate and canonical checklist
- MILESTONE_ID: _pending_
- BLUEPRINT_REF: _pending_
- PR: https://github.com/natbkgift/flowbiz-client-ai-builder/pull/55

**Evidence Links**
- https://github.com/natbkgift/flowbiz-client-ai-builder/pull/55

## PR #59: Enforce semantic references in audit documentation per governance framework
- MILESTONE_ID: PR-059
- BLUEPRINT_REF: BLUEPRINT — Governance Controls / Reference Style Remediation
- PR: https://github.com/natbkgift/flowbiz-client-ai-builder/pull/59

**Evidence Links**
- https://github.com/natbkgift/flowbiz-client-ai-builder/pull/59/checks
- https://github.com/natbkgift/flowbiz-client-ai-builder/pull/59/files
- docs/audit/EVIDENCE_INDEX.md

**Changes:**
- Added `agent-next-pr-ready` to required status checks in CODEX_AUTORUN_PROMPT_v1.2_CANONICAL.md.
- Hardened PR #59 body metadata and audit wording to match canonical prompt requirements.

## PR #TBD: Implement governance controls - semantic reference updates
- MILESTONE_ID: Governance
- BLUEPRINT_REF: CODEX_AUTORUN_PROMPT_v1.2.1_CANONICAL — Reference Style
- PR: https://github.com/natbkgift/flowbiz-client-ai-builder/pull/TBD

**Evidence Links**
- https://github.com/natbkgift/flowbiz-client-ai-builder/pull/TBD

**Changes:**
- Updated SOC2_Audit_Pack_FlowBiz_AI_Builder.md to use semantic references
- Updated AUDIT_PACK_PATCH_NOTES.md to use semantic references
- Updated PR_11-13_VERIFICATION_REPORT.md to use semantic references
- All numeric section references converted to semantic references per CODEX_AUTORUN_PROMPT_v1.2.1_CANONICAL.md — Reference Style guidance
- Tests pass: 104 passed
- Linting passes: All checks passed

## PR #62: PR-15: Evidence Model + Artifact Registry
- MILESTONE_ID: PR-15
- BLUEPRINT_REF: BLUEPRINT — PR-15: Evidence Model + Artifact Registry
- PR: https://github.com/natbkgift/flowbiz-client-ai-builder/pull/62

**Evidence Links**
- https://github.com/natbkgift/flowbiz-client-ai-builder/pull/62
